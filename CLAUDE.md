...
Single-file pure Python application (`src/cc_dashboard/__main__.py`, ~1050 lines). Zero external dependencies — stdlib only. Cross-platform: Linux (`/proc`) and macOS (`ps`, `lsof`) for process discovery.

### Key data flow

1. `get_running_claude_sessions()` — scans `ps` + `/proc` (Linux) or `ps` + `lsof` (macOS) to find Claude processes
